...
whisperCreateButton.addEventListener("click", (event) => {
  ...
  if (message) {
    createWhisper(message, user.id).then(refreshAllUI);
  }
});
...
const refreshWhispers = (data) =>
  (whispers.innerHTML = data
    .reverse()
    .map((whisper) => {
      return `
        <article data-id="${whisper.id}">
            ...
            <p class="meta">
                ...
                <span class="date">at ${new Date(
                  whisper.creationDate
                ).toLocaleString()}</span>
            </p>
        </article>`;
    })
